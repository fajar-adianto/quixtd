...
#include "auth/qx_td_auth_state_factory.h"
#include "common/qx_td_request.h"
#include "connections/qx_td_connection_state_factory.h"
#include "requests/qx_td_close_request.h"
#include "../auxdb/auxdb.h"
#include "../auxdb/avatarmaptable.h"
...
QJsonObject execTd(const QJsonObject &json, const bool debug)
{
    ...
    QSharedPointer<Handle> tdlib = QxTdHandle::instance();
...
QxTdClient::QxTdClient(QObject *parent)
    : QObject(parent)
    , m_worker(new QThread)
    , m_authState(nullptr)
    , m_connectionState(nullptr)
    , m_tagcounter(0)
    ...
    , m_debug(false)
{
    ...
    connect(this, &QxTdClient::updateOption, this, &QxTdClient::handleUpdateOption);

    connect(thread, &QThread::started, worker, &QxTdWorker::run);
    connect(worker, &QxTdWorker::finished, worker, &QxTdWorker::deleteLater);
    connect(worker, &QxTdWorker::destroyed, thread, &QThread::quit);
...
static QPointer<QxTdClient> s_tdclient;
QxTdClient *QxTdClient::instance()
{
    if (s_tdclient.isNull()) {
        s_tdclient = new QxTdClient();
    }
    return s_tdclient;
}

QxTdAuthState *QxTdClient::authState() const
{
    return m_authState;
}

QxTdConnectionState *QxTdClient::connectionState() const
{
    return m_connectionState;
}

void QxTdClient::send(QxTdRequest *obj)
{
    send(obj->marshalJson());
}

void QxTdClient::send(const QJsonObject &json)
{
    if (json.isEmpty()) {
        qDebug() << "Empty Json object, nothing to send?";
        return;
    }
    auto future = QtConcurrent::run(sendTd, json, m_debug);
    Q_UNUSED(future); // Reserved for future use.
}

QFuture<QxTdResponse> QxTdClient::sendAsync(QxTdRequest *obj, void (QxTdClient::*signal)(QJsonObject))
{
    const QString tag = getTag();
    QJsonObject data = obj->marshalJson();
    data["@extra"] = tag;
    QFuture<QxTdResponse> f = QtConcurrent::run([this](void (QxTdClient::*s)(QJsonObject), const QJsonObject &data, const QString &tag) -> QxTdResponse {
        ...
        QMetaObject::Connection con1 = QObject::connect(QxTdClient::instance(), s, respSlot);
        QMetaObject::Connection con2 = QObject::connect(QxTdClient::instance(), &QxTdClient::error, respSlot);
...
QFuture<QJsonObject> QxTdClient::exec(QxTdRequest *obj)
{
    return exec(obj->marshalJson());
}

QFuture<QJsonObject> QxTdClient::exec(const QJsonObject &json)
{
    return QtConcurrent::run(execTd, json, m_debug);
}

void QxTdClient::handleRecv(const QJsonObject &data)
{
...
void QxTdClient::init()
{
    ...
    m_events.insert(QStringLiteral("updateConnectionState"), [=](const QJsonObject &data) {
        ...
        m_connectionState = QxTdConnectionStateFactory::create(data, this);
    ...
    m_events.insert(QStringLiteral("updateChatIsBlocked"), [=](const QJsonObject &data) {
        emit updateChatIsBlocked(data);
    });
    m_events.insert(QStringLiteral("foundChatMessages"), [=](const QJsonObject &data) {
        emit foundChatMessages(data);
    });
}

void QxTdClient::handleUpdateOption(const QJsonObject &json)
{
...
QString QxTdClient::getTag()
{
    m_tagcounter++;
    return QString("req-%1").arg(QString::number(m_tagcounter));
}

QVariant QxTdClient::getOption(const QString name)
{
...
void QxTdClient::setAvatarMapEntry(const qint64 id, const QString path)
{
    if (path != "")
        m_auxdb.getAvatarMapTable()->setMapEntry(id, path);
}

void QxTdClient::removeMapEntry(const qint64 id)
{
    m_auxdb.getAvatarMapTable()->removeMapEntry(id);
    m_postalClient.setCount(m_auxdb.getAvatarMapTable()->getTotalUnread());
}

void QxTdClient::setUnreadMapEntry(const qint64 id, const qint32 unread_count)
{
    m_auxdb.getAvatarMapTable()->setUnreadMapEntry(id, unread_count);
    m_postalClient.setCount(m_auxdb.getAvatarMapTable()->getTotalUnread());
}

void QxTdClient::clearNotificationFor(const qint64 id) {
    QStringList tags;
